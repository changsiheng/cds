...
func Export(ctx context.Context, db gorp.SqlExecutor, key string, envName string, encryptFunc sdk.EncryptFunc) (exportentities.Environment, error) {
	...
	variables, err := GetAllVariable(db, key, envName, WithClearPassword())
	if err != nil {
		return exportentities.Environment{}, sdk.WrapError(err, "cannot load variables value")
	}
	env.Variable = variables

	// Reload key
	if err := LoadAllDecryptedKeys(ctx, db, env); err != nil {
